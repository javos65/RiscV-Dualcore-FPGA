...
module ahbl1 (ahbl_hclk_i, 
    ...
    ahbl1_ipgen_lscc_ahbl_interconnect #(.TOTAL_MASTER_CNT(1),
            .TOTAL_SLAVE_CNT(3),
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module ahbl1_ipgen_lscc_ahbl_arbiter #(parameter TOTAL_MASTER_CNT = 8, 
    ...
    generate
        if ((ARBITER_SCHEME == 3'b0)) 
            begin : round_robin
                ...
                ahbl1_ipgen_lscc_ahbl_fair_arb #(.NUM_REQ(TOTAL_MASTER_CNT),
    ...
    generate
        if ((ARBITER_SCHEME == 3'b1)) 
            begin : fixed_prio
                ...
                // Priority Arbiter
                // -----------------
                ahbl1_ipgen_lscc_ahbl_prio_arb #(.M(M)) u_lscc_ahbl_fp_gnt (//AUTOINSTPARAM*/
                        // Parameters
                        //AUTOINST*/
                        // Outputs
                        .gnt_o(gnt[(M - 1):0]), 
...
// ==============================================================================
module ahbl1_ipgen_lscc_ahbl_prio_arb #(parameter M = 32) (
    // -----------------------------------------------------------------------------
    // Module Parameters
    // -----------------------------------------------------------------------------
    // ------------------------------------------------------------------------------
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module ahbl1_ipgen_lscc_ahbl_arbmux #(parameter TOTAL_MASTER_CNT = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter M_PRIO_WIDTH = clog2(TOTAL_MASTER_CNT), 
        parameter [((TOTAL_MASTER_CNT * M_PRIO_WIDTH) - 1):0] M_PRIO_IDX = {4'd8,
                4'd7,
    ...
    ahbl1_ipgen_lscc_ahbl_arbiter #(.TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module ahbl1_ipgen_lscc_ahbl_bus #(parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
        parameter DATA_WIDTH = 32, 
    ...
    // AHB-Lite Decoder
    // -----------------
    ahbl1_ipgen_lscc_ahbl_decoder #(.F(F),
            .M_ADDR_WIDTH(M_ADDR_WIDTH),
            .FULL_DECODE_EN(FULL_DECODE_EN),
    ...
    // ---------------------
    ahbl1_ipgen_lscc_ahbl_multiplexor #(.M_ADDR_WIDTH(M_ADDR_WIDTH),
            .DATA_WIDTH(DATA_WIDTH),
            .TOTAL_SLAVE_CNT(TOTAL_SLAVE_CNT),
    ...
    // -----------------------
    ahbl1_ipgen_lscc_ahbl_default_slv #(.DATA_WIDTH(DATA_WIDTH)) u_lscc_ahbl_default_slv (//AUTOINSTPARAM*/
            // Parameters
            //AUTOINST*/
            // Outputs
...
// =============================================================================
module ahbl1_ipgen_lscc_ahbl_crossbar #(parameter F = 8, 
    ...
    generate
        ...
        begin : input_stage_reg
            ...
            // Input Stage
            // -------------    
            ahbl1_ipgen_lscc_ahbl_input_stage #(.M_ADDR_WIDTH(M_ADDR_WIDTH)) u_lscc_ahbl_input_stage (/*AUTOINSTPARAM*/ // Parameters
                    /*AUTOINST*/ // Outputs
    ...
    generate
        ...
        begin : bus
            ...
            // AHB-Lite Bus
            // -------------
            ahbl1_ipgen_lscc_ahbl_bus #(.M_ADDR_WIDTH(M_ADDR_WIDTH),
                    .FULL_DECODE_EN(FULL_DECODE_EN),
                    .DATA_WIDTH(DATA_WIDTH),
                    .TOTAL_SLAVE_CNT(TOTAL_SLAVE_CNT),
                    .OUTREG_EN(OUTREG_EN),
    ...
    generate
        ...
        begin : arbiter_mux
            ...
            // -----------------------------
            ahbl1_ipgen_lscc_ahbl_arbmux #(.TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
                    .M_ADDR_WIDTH(M_ADDR_WIDTH),
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module ahbl1_ipgen_lscc_ahbl_decoder #(parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
        parameter TOTAL_SLAVE_CNT = 32, 
        parameter S0_FRAGMENT_EN = 4'd1, 
    ...
    generate
        ...
        begin : genblk1
            if (M_CONNECT_EN[i]) 
                begin : genblk1
                    ahbl1_ipgen_lscc_ahbl_decoder_prim #(.F(F),
                            .M_ADDR_WIDTH(M_ADDR_WIDTH),
                            .FULL_DECODE_EN(FULL_DECODE_EN),
...
// ==============================================================================
module ahbl1_ipgen_lscc_ahbl_decoder_prim #(parameter F = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
    ...
    generate
        ...
        begin : genblk1
            ahbl1_ipgen_lscc_ahbl_decoder_comp #(.FULL_DECODE_EN(FULL_DECODE_EN),
...
// ==============================================================================
module ahbl1_ipgen_lscc_ahbl_decoder_comp #(parameter FULL_DECODE_EN = 1, 
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module ahbl1_ipgen_lscc_ahbl_default_slv #(parameter DATA_WIDTH = 32) (
    // -----------------------------------------------------------------------------
    // Module Parameters
    // -----------------------------------------------------------------------------
    // ------------------------------------------------------------------------------
...
// Module : fair_arb
//==========================================================================
module ahbl1_ipgen_lscc_ahbl_fair_arb #(parameter NUM_REQ = 4, 
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module ahbl1_ipgen_lscc_ahbl_multiplexor #(parameter M_ADDR_WIDTH = 32, 
        parameter DATA_WIDTH = 32, 
        parameter TOTAL_SLAVE_CNT = 32, 
        parameter OUTREG_EN = 0) (
    // -----------------------------------------------------------------------------
    ...
    assign hready_mstr_w = (active_tx_r ? ahbl_hreadyout_slv_o : ahbl_hready_slv_i) ; 
    generate
        always
            @(*)
            begin
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module ahbl1_ipgen_lscc_ahbl_input_stage #(parameter M_ADDR_WIDTH = 32) (
    // -----------------------------------------------------------------------------
    // Module Parameters
    // -----------------------------------------------------------------------------
    // ------------------------------------------------------------------------------
...
/* >>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     ------------------------------------------------------------------
 */
module ahbl1_ipgen_lscc_ahbl_interconnect #(parameter F = 8, 
        parameter TOTAL_MASTER_CNT = 8, 
        parameter TOTAL_SLAVE_CNT = 8, 
        parameter M_ADDR_WIDTH = 32, 
        parameter FULL_DECODE_EN = 1, 
    ...
    generate
        ...
        if (((TOTAL_MASTER_CNT > 1) && (TOTAL_SLAVE_CNT >= 1))) 
            begin : ahbl_lite_crossbar
                ...
                // AHB-Lite Crossbar
                // ------------------
                ahbl1_ipgen_lscc_ahbl_crossbar #(.TOTAL_MASTER_CNT(TOTAL_MASTER_CNT),
                        .TOTAL_SLAVE_CNT(TOTAL_SLAVE_CNT),
                        .M_ADDR_WIDTH(M_ADDR_WIDTH),
                        .FULL_DECODE_EN(FULL_DECODE_EN),
                        .DATA_WIDTH(DATA_WIDTH),
